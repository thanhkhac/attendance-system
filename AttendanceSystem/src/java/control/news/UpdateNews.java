...
public class UpdateNews extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            boolean updateResult = news.updateNews(newId, title, content, filePath, dateTime, createBy);
            String redirectURL = request.getContextPath() + "/GetAllNewsByHR";

            if (updateResult) {
                redirectURL += "?successMessage=" + URLEncoder.encode("News successfully updated!", "UTF-8");
            } else {
                redirectURL += "?errorMessage=" + URLEncoder.encode("Failed to update news. Please try again.", "UTF-8");
            }
