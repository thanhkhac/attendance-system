 ...
 * @author nguye
 */
@WebServlet(name="DenyResignationRequestServlet", urlPatterns={"/DenyResignationRequestServlet"})
public class DenyResignationRequestServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if (button_value.startsWith("Deny")) {
            ...
            if (acc.getRoleID() == 4) { // role
                ...
                if (rrDao.approvalOfResignationByManager(0, acc.getEmployeeID(), resignationRequestID_raw)){  
                    response.sendRedirect("ViewResignationRequestForManager.jsp");
                }
            } else if (acc.getRoleID() == 2) {
                // Duyet don theo role quan li nhan su
                out.println(" || 5 Role-ID-2(Quanlinhansu) : " + acc.getRoleID());
                if (rrDao.approvalOfResignationByHR(0, acc.getEmployeeID(), resignationRequestID_raw)) { 
                    response.sendRedirect("ViewResignationRequestForHR.jsp");
                }
            }
