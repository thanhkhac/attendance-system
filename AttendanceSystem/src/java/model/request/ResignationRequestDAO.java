...
public class ResignationRequestDAO extends dbhelper.DAOBase {
    ...
    public ArrayList<ResignationRequestDTO> getRegisnationRequestByDepartment(int departID) {
...
//        if (connection != null) {
            try {
                String sql = "SELECT ResignationRequestID,\n"
                        ...
                        + "WHERE e.DepartmentID like '%" + departID + "%'\n"
                        + "ORDER BY ManagerApprove ASC , ResignationRequestID ASC";
                ps = connection.prepareStatement(sql);
                rs = ps.executeQuery();
                while (rs.next()) {
...
//        if (connection != null) {
            try {
                String sql = "SELECT ResignationRequestID,\n"
                        ...
                        + "ORDER BY HrApprove ASC , ResignationRequestID ASC";
                ps = connection.prepareStatement(sql);
                rs = ps.executeQuery();
                while (rs.next()) {
                    int resignation = rs.getInt("ResignationRequestID");
...
//        ArrayList<ResignationRequestDTO> list1 = rrDao.getRegisnationRequestForHR(1);
        ArrayList<ResignationRequestDTO> list2 = rrDao.getRegisnationRequestByDepartment(2);
